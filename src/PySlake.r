...
f<-'../data/output/processed_aggregates.csv'
df<-read.csv(f)
#filter out "bad" images - weird Area values
mat_area<-df%>%dplyr::select(-Column,-Row,-Well,-Sample,-Rep)
mat_area<-mat_area%>%t()%>%as.data.frame()%>%dplyr::filter(if_all(starts_with("V"), ~ . < 300))
...
ggsave(paste0(substr(f,1,(str_length(f)-4)),'.png'),p)
...
df<-df[order(df$Initial_Area),]%>%filter(Initial_Area>10)
p<-ggplot(df)+geom_boxplot(aes(x=SI600,colour=Sample))
p
ggsave('../data/output/PySlakeBoxplotSI600.png',p)

p<-ggplot(df)+geom_boxplot(aes(x=Initial_Area,colour=Sample))
p
ggsave('../data/output/PySlakeBoxplotA0.png',p)
write.csv(file = '../data/output/PySlakeOutput.csv',df)
